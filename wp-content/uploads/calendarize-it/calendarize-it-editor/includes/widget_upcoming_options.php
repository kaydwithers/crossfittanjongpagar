...
//----- Date range -----------------------------
		$i++;
		$t[$i]=(object)array();
		$t[$i]->id 			= $box_prefix.'-daterange'; 
		$t[$i]->label 		= __('Date range','rhc');
		$t[$i]->options = array();	
			
		$t[$i]->options = $this->add_font_options( $t[$i]->options, array(
			'prefix'	=> $box_prefix.'_date_range_',
			'selector'	=> implode(',',array(
				$item_selector.' .rhc-widget-date-range',
				$item_selector.' .rhc-widget-show-on-time'
			)),	
			'labels'	=> (object)array(
				'family'	=> __('Date range font','rhc'),
				'size'		=> __('Size','rhc'),
				'color'		=> __('Color','rhc')				
			)
		));				
