...
class rhc_styles_and_scripts {
	...
	function admin_and_frontend_scripts(){
		...
		if( $this->debugging_js_css ){
			...
			wp_register_script( 'calendarize', RHC_URL.'js/calendarize.js', $dependency,'3.4.2.1',$this->in_footer);
		}else{
			...
			wp_register_script( 'calendarize', RHC_URL.'js/frontend.min.js', $dependency,'3.4.2.1',$this->in_footer);
		}
	...
	function admin_only_scripts(){
		...
		wp_register_script( 'calendarize-metabox', RHC_URL.'js/calendarize_metabox_rrule.js', array('jquery'),'1.2.6.3');				
